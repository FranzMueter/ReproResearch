...
```{r a}
# generate histogram of ozone data
hist(enviro$ozone)

# generate histogram of third root ozone data
hist(enviro$ozone^(1/3))

# The difference between raw and third root data is the latter normalizes the ozone data distribution, therefore changing the shape of the histogram toward normal distribution.
# We want to transform the ozone data in this manner to take non-linear relationships between ozone and other measurement variables and make them linear when doing regression analysis.
```
...
```{r b, eval=TRUE}
# Fit lowess model of transformed ozone by wind and temperature
ozo1 <- loess(ozone^(1/3) ~ wind + temperature, enviro, span = 1, degree = 2)
summary(ozo1)
```
...
```{r c}
# extract regression coefficients: fitted and resid
ozo1.fitted <- fitted(ozo1)
ozo1.resid <- resid(ozo1)

# plot observed, transformed values vs. predicted, fitted values
plot(ozo1.fitted, enviro$ozone^(1/3), xlab = "Fitted values", 
  ylab = "Observed values", cex.lab=1.5)
abline(0,1, col=2) # 1:1 line
```
...
```summary(ozo1)```
```{r d}
# pull out the ozone standard error
summary(ozo1)
# Residual Standard Error: 0.501
# alternatively
str(ozo1)

# Create or extract summary values s, p, n
(s <- ozo1$s)
(p <- ozo1$enp)  
(n <- ozo1$n)

# calculate the residuals instead, should be close to s
(ozo1.resid2 <- sqrt(sum(resid(ozo1)^2)/(n-p)))
# 0.4971813 is close to (s <- ozo1$s) = 0.501044
```
...
```{r e}
# plot fitted values + residual values agains observed
plot(ozo1.fitted + ozo1.resid, enviro$ozone^(1/3), xlab = "Fitted values", ylab = "Observed values", cex.lab=1.5)
abline(0,1, col=2) # 1:1 line)
```
...
should roughly fall on a straight line:   
   ...
   ```qqline(resid(ozo1))```
```{r f}
# create diagnostic plots frame
par(mfrow=c(2,2))

# 1. Plot residuals (on y-axis) against fitted values, add a horizontal line at 0 (```abline(h=0)```)
plot(ozo1.fitted, ozo1.resid, xlab = "Fitted values", ylab = "Residuals", cex.lab=1.5)
abline(h=0, col = 2) # horizontal line)

# 2. Plot residuals against wind and add line at 0
plot(enviro$wind, ozo1.resid, xlab = "Wind", ylab = "Residuals", cex.lab=1.5)
abline(h=0, col = 2) # horizontal line)

# 3. Plot residuals against temperature and add line at 0
plot(enviro$temperature, ozo1.resid, xlab = "Temperature", ylab = "Residuals", cex.lab=1.5)
abline(h=0, col = 2) # horizontal line)

# 4. Create a ```qqplot``` that compares the quantiles of the residual distribution against the corresponding quantiles of a normal distribution. If residuals are normally distributed, they should roughly fall on a straight line:   ```qqnorm(resid(ozo1))```  
qqnorm(resid(ozo1))
abline(0,0.5, col = 2) # 1:1 line)
```
...
```{r g}
# find max and min of wind and temperature
max.wind <- max(enviro$wind)
min.wind <- min(enviro$wind)
max.temp <- max(enviro$temperature)
min.temp <- min(enviro$temperature)

# create vector sequences across wind and temperature
wind.fit.seq <- seq(min.wind, max.wind, length.out=50)
temp.fit.seq <- seq(min.temp, max.temp, length.out=50)

# create matrix of wind and temp sequences
grd <- expand.grid(list(wind=wind.fit.seq, temperature=temp.fit.seq))
str(grd)

# predict the values of ozo1 using grd
ozo1.fit <- predict(ozo1, newdata = grd)

# re-set figure
par(mfrow=c(1,1))

# Use the fitted lowess model to get predicted values for each x- and y-value in 'grd'
image(wind.fit.seq, temp.fit.seq, ozo1.fit, col = topo.colors(80), 
      xlab="Wind Speed (mph)", ylab="Temperature (F)",
      main = "Cube Root Ozone (cube root ppb)", cex.axis=1.2, cex.lab=1.4)
# Add contours:
contour(wind.fit.seq, temp.fit.seq, ozo1.fit, add=T, labcex=1.5)
# show when measurements were taken:
points(enviro$wind, enviro$temp, pch=16, cex=0.5, col=2) 
```

The model does not fit very well at high wind speeds and tempertures as there are no real data to for comparison. I would be skeptical of these large(ish) ozone gradients in high wind and temperature locations.
...
```{r h, eval=TRUE}
# use contourplot to do the same thing
...
```{r i}
# extend the model to include a third variable, ratiation
ozo2 <- loess(ozone^(1/3) ~ wind+temperature+radiation, data = enviro, span=1, degree=2)
summary(ozo2)
summary(ozo1)

# approximate F-test to compare model fits
anova(ozo1, ozo2)
```
...
1) RRS decreased from 25.749 in ozo1 to 19.790 in ozo2, the more complex model. Therefore, the relative reduction in RSS (F-test) is 4.4603.
2) One extra "equivalent parameter" was needed, i.e. the inclusion of radiation in the model.
...
The probability we would get an F value as large as the observed is 0.0006947. We can conclude that ozo2 is a better model.
...
# create new radiation sequence and reset grd with radiation
head(enviro)
rad.fit.seq <- seq(min(enviro$radiation), max(enviro$radiation),
                   length = 4) 
grd <- expand.grid(list(wind=wind.fit.seq, temperature = temp.fit.seq, radiation=rad.fit.seq))
head(grd)
summary(grd)

# predict ozo values and add to matrix grd
pred <- predict(ozo2, grd)

# compare to pred and grd
str(pred)
dim(pred)
str(grd)
dim(grd)
 
# assign pred values to grd, named fit
grd[, "fit"] <- as.vector(pred)

# contour plot wind and temp by radiation
...
Estimated ozone levels increase with increasing radiation as seen across plots.
